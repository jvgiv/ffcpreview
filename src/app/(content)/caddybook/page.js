...
export default function CaddyBook() {
  ...
  const originalNotesRef = useRef(false);
  ...
  useEffect(() => {
    ...
    try {
      ...
      setNotes(parsed);
      originalNotesRef.current = false;
    } catch (e) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    emailjs
      .send(serviceId, templateId, notes, { publicKey: publicKeyEnv })
      .then(() => {
        setCurrent(13);
